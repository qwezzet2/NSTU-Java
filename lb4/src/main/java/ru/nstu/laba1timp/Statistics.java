...
public class Statistics {
    ...
    public void startAction() {
        ...
        if (restartFlag || firstActionFlag) {
            hab.clearObjects();
            // Clear ImageViews from the pane on the JavaFX Application Thread
            Platform.runLater(() -> mainController.getPane().getChildren().removeIf(node -> node instanceof javafx.scene.image.ImageView));
            seconds = -1;
            minutes = 0;
            timer = new Timer();
            restartFlag = false;
        ...
        // Resume AI threads if their respective ON buttons are selected
        // This happens when resuming after clicking Cancel in the statistics dialog
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                Platform.runLater(() -> {
                    updateTimer();
                    // Object creation/deletion logic remains in update
    ...
    public void stopAction() {
        ...
        pauseMusic();
        // Check if the "Show Information" checkbox is selected in the Controller
        if (mainController.btnShowInfo.isSelected()) {
            ...
            if (result.get() == ButtonType.OK) {
                // If OK is pressed, set restartFlag to true for a full restart on next start
                restartFlag = true;
                // Re-enable simulation parameter input fields and priority combo boxes
                ...
                mainController.boxDevPriority.setDisable(false);
                mainController.boxManPriority.setDisable(false);
                stopMusic(); // Stop music completely

                // NOTE: Buttons ON/OFF intellect are not re-enabled here because clicking OK
                // implies a full stop, and they will be enabled again by clickStart
                // when the user chooses to start a new simulation.

            } else { // If Cancel is pressed
                // Set main Start/Stop button states for resumed simulation
                ...
                // Explicitly re-enable the ON/OFF intellect buttons
                mainController.btnDevIntellect.setDisable(false);
                mainController.btnManIntellect.setDisable(false);

                // Resume the simulation (this will also resume AI threads if ON buttons are ON)
                startAction();
            }
        } else { // If the "Show Information" checkbox is NOT selected
            stopMusic(); // Simply stop the music
            // If the dialog is not shown, we assume a full stop like pressing OK in the dialog
            restartFlag = true;
            mainController.fieldN1.setDisable(false);
            mainController.fieldN2.setDisable(false);
            mainController.fieldLifeTimeDev.setDisable(false);
            ...
            mainController.boxDevPriority.setDisable(false);
            mainController.boxManPriority.setDisable(false);
            // Buttons ON/OFF intellect remain disabled from Controller.clickStop()
        }
    }
}